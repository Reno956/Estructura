...
public class Raiz {
    ...
    public void eliminar(int valor) { //elimina de la lista el valor y vuelve a crear el arbol
        boolean encontrado = false;//se crea una bandera
        int j = 0;
        //mediante un for se busca el elemento en el ArrayList ingresados
        for (int i=0; i<Lista.ingresados.size() && !encontrado; i++) {
            //si el valor coincide la bandera se vulve true y termina el bucle
            if (Lista.ingresados.get(i) == valor) {
                encontrado = true;
                j = i;//se obtiene el indice del elemento encontrado
            }
        }
        //si la bandera es true se elimina el elemento en el indice encontrado
        if (encontrado==true) {
            Lista.ingresados.remove(j);
        } else {//caso contrario se emite el mensaje
            System.out.println("El valor a eliminar no se encuentra en el arbol B");
        }
        //se crea un nuevo ArrayList con los valores restantes
        ArrayList<Integer> auxiliar = Lista.ingresados;
        Lista.ingresados = new ArrayList<Integer>();
        primerNodo = new Nodo();//se setea primerNodo
        primerNodo.tengoHijos = false;//se elimina los hijos
        //se vuelve a insertar los valores en el arbol original
    ...
    public boolean buscar(int valor){
        boolean esta = false;//se crea una bandera
        //mediante un for se busca el elemento en el ArrayList ingresados
        for(int i = 0; i < Lista.ingresados.size() && !esta; i++){
            //si el valor coincide la bandera se vulve true y termina el bucle
            if(Lista.ingresados.get(i) == valor){
                esta = true;
                //se emite el mensaje y se retorna el valor de la bandera true
                System.out.println("El elemento buscado si se encuentra en el arbol B");
                return esta;
            }
        }
        //se emite el mensaje y se retorna el valor de la bandera false
